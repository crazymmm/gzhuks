                   ...
                   if($userAuthority['msg'][0]['flag']== 0){
                       if($id==$userId){
                           ...
                           if (!$this->checkMain('password',$edit_password)){
                               return new JsonModel(array('result' => false,'msg' => '异常:新密码需要大小写字母跟数字组成并且长度在6-16字符'));
                           }
                           if (!$this->checkMain('password',$edit_passwordVerify)){
                               return new JsonModel(array('result' => false,'msg' => '异常:确认密码需要大小写字母跟数字组成并且长度在6-16字符'));
                           }
                           ...
                           //    var_dump($user_id['msg'][0]['password']);exit;
                           if($ret){
                               ...
                               if(!empty($_FILES['edit_avatarfile']['tmp_name'])){
                                   ...
                                   $headpic = $ava_filenamePath . "/s_"  . md5($userId) . '_avatar.' . $_FILES['edit_avatarfile']['extension'];
                                       ...
                                       $useModel=new User($this->getDbAdapter());
                                       $ret=$useModel->selectUserById($id);
                                        if(!empty($ret['msg'][0]['headpic'])){
                                        $headpic=$ret['msg'][0]['photo'];
                                           }else{
                                               /*默认头像*/
                                        $headpic = "/data/avatarResource/common/default.jpg";
                                                 }
                               ...
                               return new JsonModel(array('result'=>false,"msg"=>"旧密码错误"));
                           }
