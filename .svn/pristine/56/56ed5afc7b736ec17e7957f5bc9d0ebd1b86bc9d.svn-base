...
(function (factory) {
    ...
    //noinspection HtmlUnknownAttribute
    tBtnLink = '<a href="{href}" id="{uploadInputId}" tabindex="500" title="{title}" class="{css}" {status}>{icon} {label}</a>';
    //noinspection HtmlUnknownAttribute
            ...
            switch (type) {
                ...
                case 'upload':
                    ...
                    if (self.isUploadable && !self.isDisabled) {
                        ...
                        tmplt = tmplt.replace('{uploadInputId}', self.uploadInputId);
                    } else {
                        btnType = 'submit';
                    }
                    break;
