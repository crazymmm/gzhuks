...
//             return $view;
    ...
    public function selectallcourseAction(){
        try {
            if($this->getRequest()->isPost()){
                ...
                $user = new User($this->getDbAdapter());
                $Ret = $user->selectallcourse();
                return new JsonModel(array("result"=>$Ret));
    ...
    public function addmyselectcourseAction(){
        try{
            ...
            $userId = $user_session->userId;
            if ($userId==null||$userId==""){
                return new JsonModel(array('result'=>false,'msg'=>'nologin'));
            }
            ...
            if($this->getRequest()->isPost()){
                ...
                if ($ishava['result']) {
                    if (sizeof($ishava['msg'])==0) {
                          ...
                          return new JsonModel(array('result'=>true,'msg'=>'选课成功'));
                    }else{
                        $this->getDbAdapter()->getDriver()->getConnection()->rollback();
                        return new JsonModel(array('result'=>false,'msg'=>'你已经选读该课程'));
                    }
                }else{
                    $this->getDbAdapter()->getDriver()->getConnection()->rollback();
    ...
    public function deletemycourseAction(){
        ...
        if($this->getRequest()->isPost()){
            ...
            if(!$Ret['result']){
                 $this->getDbAdapter()->getDriver()->getConnection()->rollback();
                 return new JsonModel(array('result'=>false,'msg'=>$Ret['msg']));
            }
            $this->getDbAdapter()->getDriver()->getConnection()->commit();
            return new JsonModel(array('result'=>true,'msg'=>'取消成功'));
       }else{
            $response = $this->getResponse();
            $response->getHeaders()->addHeaderLine('Location', "../../../../../html/common/404.html");
            $response->setStatusCode(302);
